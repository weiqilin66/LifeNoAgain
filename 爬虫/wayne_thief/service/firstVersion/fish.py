...
class CrawlFish(object):
    ...
    def search(self, obj):
        ...
        time.sleep(1)
        app.tapByPercentage(my_constants[phone]['最低价'])
        time.sleep(0.5)
        app.pressNum(low_price)
        app.tapByPercentage(my_constants[phone]['最高价'])
        time.sleep(0.5)
        app.pressNum(high_price)
        # 分辨率之外的点击 os.popen('adb shell input tap x y ')
        os.popen(my_constants[phone]['筛选确定'])
        time.sleep(1)
    ...
    def analyse(self, info, obj):
        ...
        if end_price <= high_price:
            ...
            os.popen(my_constants[phone]['黏贴'])     # os.popen(cmd)
